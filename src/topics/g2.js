...
// @@SECTION:anims@@
     ...
     },
     {
      ...
      get formula() { const {A,B,C}=g2MainPts(); const lAC=Math.round(Math.hypot(C[0]-A[0],C[1]-A[1])/8),lBC=Math.round(Math.hypot(C[0]-B[0],C[1]-B[1])/8),lAB=Math.round(Math.hypot(B[0]-A[0],B[1]-A[1])/8); return Math.abs(lAC-lBC)<2?`等腰三角形：a=b=${Math.min(lAC,lBC)}，c=${lAB}`:`不等边三角形：a=${lBC}  b=${lAC}  c=${lAB}`; },
      dur: 100, noAutoFit: true,
