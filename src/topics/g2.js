    ...
    if (step===2) {
     ...
     const tDefs=[
      { label:'等边三角形', desc:'三边相等', formula:'a = b = c',
        ...
        A:[5,185], B:[84,185], C:[44,117], tk:[1,1,1] },
      { label:'等腰三角形', desc:'两腰相等', formula:'a = b ≠ c',
        color:'#C07020', fill:'rgba(220,150,30,0.18)',
        A:[102,185], B:[190,185], C:[146,108], tk:[1,1,2] },
      { label:'不等边三角形', desc:'三边各不等', formula:'a ≠ b ≠ c',
        color:'#A03060', fill:'rgba(200,50,110,0.18)',
        A:[207,187], B:[289,187], C:[244,122], tk:[0,0,0] },
     ];
     tDefs.forEach(({label,desc,color,fill,A,B,C,tk},i)=>{
      const active=i===typeIdx;
      ctx.save(); ctx.globalAlpha=active?1:0.2;
      g2DrawTri(ctx,[A,B,C],active?fill:'rgba(160,160,160,0.06)',active?color:'#bbb',active?2:1);
      if (active) {
      ...
      g2Lbl(ctx,label,midX,A[1]+19,active?color:'#ccc',11,'center',active?'bold':'normal');
      if (active) g2Lbl(ctx,desc,midX,A[1]+33,color,10,'center');
      ctx.restore();
     });
