// @@SECTION:helpers@@
   ...
   function g2Frame(canvas, step, p) {
    ...
    if (step===1) {
     ...
     const clsDesc={'锐角三角形':'三个角都 < 90°','直角三角形':'一个角 = 90°','钝角三角形':'一个角 > 90°'};
     const RX=256,cw=154,ch=108,cy=(H-ch)/2-10;
     g2Box(ctx,RX,cy,cw,ch,8,'#FAFAFA',cls.color,2.5);
     g2Lbl(ctx,'按角分类',RX+cw/2,cy+16,'#777',11,'center');
     g2Lbl(ctx,cls.type,RX+cw/2,cy+50,cls.color,18,'center','bold');
     g2Lbl(ctx,clsDesc[cls.type],RX+cw/2,cy+74,'#555',11,'center');
     g2Lbl(ctx,'最大角 = '+mx+'°',RX+cw/2,cy+ch-12,cls.color,12,'center','bold');

     // Bottom type-chip row (shows which type is currently active)
     ...
     angChips.forEach(({type,color},i)=>{
      ...
      g2Box(ctx,chipX,247,chipW,22,4,active?color+'20':'#F5F5F5',active?color:'#ccc',active?2:1);
      g2Lbl(ctx,type,chipX+chipW/2,258,active?color:'#aaa',11,'center',active?'bold':'normal');
     });
     g2Lbl(ctx,'拖动滑块改变三角形——实时显示分类',W/2-14,276,'#555',10,'center');
    }

    // ── Step 2: 按边分类（交互式） ─────────────────────────────────────────────
    if (step===2) {
     const {A,B,C}=g2MainPts();
     // Side lengths (normalized by /8 for readable numbers)
     const lAB=Math.round(Math.hypot(B[0]-A[0],B[1]-A[1])/8);
     const lAC=Math.round(Math.hypot(C[0]-A[0],C[1]-A[1])/8);
     const lBC=Math.round(Math.hypot(C[0]-B[0],C[1]-B[1])/8);
     const EPS=2;
     let sideCls;
     if (Math.abs(lAB-lAC)<EPS&&Math.abs(lAB-lBC)<EPS) sideCls={type:'等边三角形',color:'#1A5FA8'};
     else if (Math.abs(lAC-lBC)<EPS) sideCls={type:'等腰三角形',color:'#C07020'};
     else sideCls={type:'不等边三角形',color:'#A03060'};

     // Triangle fill colored by type
     g2DrawTri(ctx,[A,B,C],sideCls.color+'18',sideCls.color,2);

     // Tick marks for equal sides
     if (sideCls.type==='等边三角形') {
      g2Tick(ctx,A,B,1,sideCls.color); g2Tick(ctx,A,C,1,sideCls.color); g2Tick(ctx,B,C,1,sideCls.color);
     } else if (sideCls.type==='等腰三角形') {
      g2Tick(ctx,A,C,1,sideCls.color); g2Tick(ctx,B,C,1,sideCls.color);
      g2Tick(ctx,A,B,2,sideCls.color);
     }

     // Side length labels at midpoints
     const bg='rgba(255,255,255,0.85)';
     g2Lbl(ctx,'c='+lAB,(A[0]+B[0])/2,(A[1]+B[1])/2+15,sideCls.color,11,'center','bold',bg);
     g2Lbl(ctx,'b='+lAC,(A[0]+C[0])/2-17,(A[1]+C[1])/2,sideCls.color,11,'center','bold',bg);
     g2Lbl(ctx,'a='+lBC,(B[0]+C[0])/2+17,(B[1]+C[1])/2,sideCls.color,11,'center','bold',bg);

     // Classification card on right
     const clsDesc2={'等边三角形':'三边相等 a=b=c','等腰三角形':'两腰相等 a=b','不等边三角形':'三边各不相等'};
     const eqLine=sideCls.type==='等腰三角形'?`a=b=${Math.min(lAC,lBC)}，c=${lAB}`:
                  sideCls.type==='等边三角形'?`a=b=c=${lAC}`:`a=${lBC}  b=${lAC}  c=${lAB}`;
     const RX=256,cw=154,ch=108,cy=(H-ch)/2-10;
     g2Box(ctx,RX,cy,cw,ch,8,'#FAFAFA',sideCls.color,2.5);
     g2Lbl(ctx,'按边分类',RX+cw/2,cy+16,'#777',11,'center');
     g2Lbl(ctx,sideCls.type,RX+cw/2,cy+50,sideCls.color,18,'center','bold');
     g2Lbl(ctx,clsDesc2[sideCls.type],RX+cw/2,cy+74,'#555',11,'center');
     g2Lbl(ctx,eqLine,RX+cw/2,cy+ch-12,sideCls.color,10,'center','bold');

     // Bottom type-chip row
     const sideChips=[{type:'等边三角形',color:'#1A5FA8'},{type:'等腰三角形',color:'#C07020'},{type:'不等边三角形',color:'#A03060'}];
     sideChips.forEach(({type,color},i)=>{
      const chipX=18+i*134, chipW=124, active=type===sideCls.type;
      g2Box(ctx,chipX,247,chipW,22,4,active?color+'20':'#F5F5F5',active?color:'#ccc',active?2:1);
      g2Lbl(ctx,type,chipX+chipW/2,258,active?color:'#aaa',11,'center',active?'bold':'normal');
     });
     g2Lbl(ctx,'拖动滑块移动顶点——实时显示边长分类',W/2-14,276,'#555',10,'center');
    }
...
// @@SECTION:anims@@
     ...
     {
      hint: '按边长分类：三边相等是等边三角形；两腰相等是等腰三角形；三边都不相等是不等边三角形。拖动滑块观察边长变化！',
      get formula() { const {A,B,C}=g2MainPts(); const lAC=Math.round(Math.hypot(C[0]-A[0],C[1]-A[1])/8),lBC=Math.round(Math.hypot(C[0]-B[0],C[1]-B[1])/8),lAB=Math.round(Math.hypot(B[0]-A[0],B[1]-A[1])/8); const E=2; if(Math.abs(lAB-lAC)<E&&Math.abs(lAB-lBC)<E) return `等边三角形：a=b=c=${lAC}`; if(Math.abs(lAC-lBC)<E) return `等腰三角形：a=b=${Math.min(lAC,lBC)}，c=${lAB}`; return `不等边三角形：a=${lBC}  b=${lAC}  c=${lAB}`; },
      dur: 100, noAutoFit: true,
