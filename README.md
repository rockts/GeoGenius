...
小学图形与几何动态演示学习工具。工具通过 SVG / Canvas 分步动画呈现公式推导和图形运动过程，配合 AI 老师讲解、变式练习题和匿名使用数据收集，帮助教师在课堂上快速展示关键几何关系。

🔗 **在线体验：[geogenius.xiaole.app](https://geogenius.xiaole.app/)**

## 功能

- **分步动画推导**：35 个知识点拆分为 3–5 步，SVG / Canvas 动画直观演示公式来源，可暂停、重播、逐步控制
- **144 个动画步骤**：自定义缓动引擎，`requestAnimationFrame` 驱动，自动适配视口大小；Canvas 交互步骤支持实时滑块参数联动
- **AI 老师对话**：开箱即用，无需配置；内置后端代理，默认使用 DeepSeek 模型；也可在设置中填入自己的 API Key 切换
- **变式练习题**：覆盖公式认识 + 应用变式（含反推未知量、实际应用情境、钝角三角形高的位置等），含即时反馈和"再来一题"
...
- **分享支持**：Open Graph / WeChat 朋友圈 meta 标签 + og.svg 预览图（1200×630），可直接分享到微信
...
| 四年级 | 角的度量、平行四边形与梯形、线段射线与直线、平行与垂直、观察物体 | 轴对称深化、平移深化、旋转深化 |
| 五年级 | 平行四边形面积、三角形面积、梯形面积、组合图形面积、长方体和正方体的认识、三角形的认识 | 图形的放大与缩小 |
| 六年级 | 圆的周长、圆的面积、圆柱表面积、圆柱体积、圆锥体积 | 位置与方向 |

**合计：35 个知识点，144 个动画步骤**
...
单文件 HTML（`index.html`），无外部框架，无构建步骤。Canvas 交互话题通过 `build.sh` 从 `src/topics/*.js` 注入：

| 类别 | 实现 |
|------|------|
| 图形渲染 | 原生 SVG（辅助函数 E/T/R/L/C）+ Canvas 2D（交互话题） |
| 动画引擎 | `requestAnimationFrame` + ease-in-out 缓动，支持速度控制 |
| 交互话题 | Canvas + 实时滑块，Cabinet 投影 3D、拖动顶点分类、方向角定位等 |
...
| 分享 | Favicon SVG data URI + OG meta + og.svg（1200×630） |

## 已修复的问题

- b2 长方形与正方形：新增对称轴演示步骤（长方形2条/正方形4条），折叠动画 + quiz 同步覆盖
- d1 角的度量：删除偏题的三角形内角和步骤，新增画角 Canvas 交互（滑块 25–155°）和估角练习 SVG 动画
- d5 观察物体：全部 4 步重做为 Canvas，Cabinet 投影长方体按 a/b/h 等比绘制，正面/俯视/侧面高亮→箭头→视图生长动画
- 长方形对称轴画法（对角线 → 横纵中线）
